...
This is, of course, easy to notice only after the fact. This post describes a series of such errors we recently made
in one of our applications. What makes it interesting was that original symptoms indicated a completely different kind
...
the threshold had been exceeded by multiple instances restarting in a short period of time.
...
go down in production) that I though maybe we had indeed failed to handle this case gracefully. I woke up the on-duty person
...
I sent messages to a few more teams who were apparently not yet aware of the issue.

My colleague from ZooKeeper team got back to me, saying that everything looked perfectly normal from his of view. Since other users seemed 
...
just a single rack to get cut off from the rest, they checked the particular hosts on which my app instances were running
and found no issue. I had checked a few side ideas in the meantime but none worked and I was at my wit’s end.
...
of each instance and if the endpoint isn’t responding, Marathon restarts that instance.

![20-second GC pauses — certainly not your average GC log](/img/articles/2018-02-09-a-comedy-of-java-errors/adventory-gc-pause-20-s.png){: .center-image }
...
## It must be a memory leak after all
...
But… this couldn’t be. We were streaming the records using RX exactly for this reason: in order to avoid loading all of the data into memory.

![Memory Sampler showed many more Ad objects than we expected](/img/articles/2018-02-09-a-comedy-of-java-errors/memory-sampler-many-ad-objects.png){: .center-image }
...
to be kept live until the end of indexing, and prevented the Garbage Collector from freeing them up earlier.
...
earlier about the app needing a 12 GB heap, which seemed strange. But on the other hand, this code had worked for almost two years and
only broke then. We were able to fix it with relative ease at this point while it would probably have taken us much more time if we tried
...
![Lots of premature tenuring](/img/articles/2018-02-09-a-comedy-of-java-errors/premature-tenuring-with-backpressure-added.png){: .center-image }
...
if I raised both bounds: `-XX:G1NewSizePercent=40` and `-XX:G1MaxNewSizePercent=90`. This did not help, it actually made matters much worse,
...
After trying a few other options, which was just as unsuccessful as my first attempt, I gave up and e-mailed Kirk Pepperdine
...
![Problem fixed — GC pauses after all changes and 4 GB heap](/img/articles/2018-02-09-a-comedy-of-java-errors/adventory-gc-pause-fixed-4-gb.png){: .center-image }
...
issue, turned out to be an error in our own code. Even after realizing this, the first steps I undertook were not well though out.
