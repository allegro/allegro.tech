...
of a part of a systems, usually with all the dependencies mocked. Integration tests also test some part of a system, but in relation to other components of the system.
...
described by Grzegorz Witkowski in his blog posts about [Acceptance testing with JBehave and Gradle](http://allegrotech.io/acceptance-testing-with-jbehave-and-gradle.html).
It provides some kind of a high level abstraction, which some people do like while the others do not.

For me tests should be short, simple and readable. It is a place where DSLs shine.
...
but at the same time we used to write our old [Selenium](http://www.seleniumhq.org/projects/webdriver/)/Java automated tests. And they were ugly...
...
continuous integration purposes for instance). It's based on Selenium WebDriver so you must remember you may use and easily migrate all the settings you used for a standard
...
In our example we'll use [gradle](http://gradle.org/) for building our project an managing its dependencies. For the start let’s clone the repository of Geb gradle example.
...
this element in a separate module and including it as needed. The definition of a module is very similar to the definition of a page. We also have a
