...
and it's mature and stable enough to be used for developing solid reliable applications.
...
One of the first features a Kotlin developer learns is the language's approach to handling `null` values.
...
the world of our code into two areas: one where `null`s are allowed and one when they aren't.
...
bytecode -- although for brevity we'll actually see equivalent Java code. Then, in the second part of this article -- for
some interesting cases we'll have a glance at the machine code generated by [JVM JIT compiler](http://openjdk.java.net/groups/hotspot/docs/HotSpotGlossary.html#JITCompilers).
...
We'll delve into the details a bit later -- for now we can make a simplification and
...
method is used. Again we'll investigate this method a bit later. For now it is enough to say that
...
Let's suppose that for a `null` value we'd like to set `username` to `"N/A"`:
...
The Kotlin code is indeed a lot shorter and more expressive. To further aid developers
...
To resolve this problem Kotlin introduces the concept of [platform types](https://kotlinlang.org/docs/reference/java-interop.html#null-safety-and-platform-types).
...
the second parameter does not have to be a variable name. In fact it's always an expression name,
