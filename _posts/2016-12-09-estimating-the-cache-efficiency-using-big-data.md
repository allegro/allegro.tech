...
purchase one of 50 millions items available on the site. To show so-called Allegro
[„listing”](http://allegro.pl/laptopy-491) page, [Solr](http://lucene.apache.org/solr/) is requested with user
query for items' identifiers, then items' data is retrieved from [Cassandra](http://cassandra.apache.org/),
which is super-fast, scalable and reliable NoSQL database.

As a team responsible for Allegro listing reliability and performance, we started to look for a solution that
decreases Cassandra load generated by our application. The first and obvious idea that appeared was to cache items in
listing application. However we did not want to build distributed cache as it adds complexity to the system
and Cassandra itself is very efficient.
...
they cannot fit into single application instance memory. Our intuition told us that caching only a small fraction of
items could be a good solution, because probably items are not requested at an equal frequency.
items could be a good solution, because probably items are not requested at an equal frequency.
We did not want to implement some caching engine with „random” parameters blindly.
...
Data analysis provided us interesting facts, which are not visible in the chart:
...
We put vertical line on some threshold (for example, 10 retrievals per item).
In the upper chart red area is amount of cached items, while total area is
amount of all items. Similarly in the lower chart, the red part represents amount of cached requests whereas
whole area is total amount of requests. As we can see, it is possible to save big percent of requests by
...
- *f(n)* - number of items found exactly *n* times during a particular period of time
- *f<sub>all</sub>* - number of all items found during a particular period of time (1,043,397 in our case)
...
- *g(n) = f(n) * n* - number of SELECTs generated by items found exactly *n* times during a particular period of time
- *g<sub>all</sub>* - number of all SELECTs generated during a particular period of time (1,577,607 in our case)
...
Here are te results for *n* up to 10 gathered from traffic to single node within 15 minutes:
...
ratio is much higher than fraction of items in cache.
...
We can describe our very simple algorithm as follows: *„We count requests for each item. When request for item appears,
...
We proved that there is no need to apply distributed cache, as it makes the overall system architecture more complex
...
- think about how you cache — it is much more than only add `@Cached` annotation in code;
- use cache in a way that reveals its efficiency optimization algorithms;
- look for cache usage also not in obvious cases.
