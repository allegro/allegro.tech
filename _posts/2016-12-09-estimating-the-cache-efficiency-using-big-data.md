...
Allegro is the largest e-commerce site in Central Europe. Every day you can browse and purchase one of 50 millions of
items available on site. To show so-called Allegro [„listing”](http://allegro.pl/laptopy-491) page,
[Solr](http://lucene.apache.org/solr/) is requested with user query for item identifiers, then items' data is
filled in from [Cassandra](http://cassandra.apache.org/), which is super-fast, scalable and reliable NoSQL database.
...
But we did not want to implement some caching engine with „random” parameters blindly.
...
Computations for 24 hours and whole traffic do not fit our case. We cannot cache items for one day, because 
