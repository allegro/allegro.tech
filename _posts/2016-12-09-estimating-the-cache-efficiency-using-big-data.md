...
50 millions of items. To show so-called Allegro [„listing”](http://allegro.pl/laptopy-491) page,
...
it cannot be fit into single application instance memory. Our intuition told us that caching only a small fraction of
items can be a good solution, because probably not every item is requested at an equal frequency.
...
So let's get all ids found in given period of time (24 hour period was choosen for first try).
...
We proved that cache can be successfully implemented in not obvious case and caching only a small part of large data
set can significantly decrease resources usage. So our final conclusions and recommendations are:
