...
Two years ago at Allegro we were using a very typical Big Data technology stack. The architecture was based
on a [Hadoop](https://hadoop.apache.org/) cluster and we were querying it with plain [Hive](https://hive.apache.org/)
queries, [Spark](https://spark.apache.org/) jobs and [Jupyter](https://jupyter.org/) notebooks. Over those last two
years we have transformed it into a more efficient and easy to use OLAP platform. 

## State of art when it comes to OLAP in Big Data

Having a setup like described above required a lot of technical knowledge and thus was not really user friendly.
Analysts who are familiar with SQL but are also less familiar with programming had to write SQL-like queries:
HQL for Hive, almost SQL for Spark etc. Both preparing and running those queries to get the expected results
were time consuming tasks which actually prevented iterative data exploration. We have speeded up the queries
...
There was an another issue — we were living in a world defined by batches. Although we have had a lot of real-time
...
specific applications. This meant that users could not access insights about ongoing experiments as the data was not
...
What was even worse — business people who were not familiar with SQL could not make any ad-hoc analysis with neither
batch nor real-time data. They had to rely on reports prepared by analysts’ teams using BI tools with already predefined
...
Apache Druid is an online analytical processing ([OLAP](https://en.wikipedia.org/wiki/Online_analytical_processing))
...
which typically leads to 10 — 100x less rows. This opened the possibilities for creating truly interactive UI
...
At the time we were installing the Druid cluster we have found out that there existed two web based applications
that could make use of it: AirBnB’s [SuperSet](https://superset.incubator.apache.org/)
and Imply’s [Pivot](https://pivot.imply.io/). We have installed both of them and immediately discovered what use cases
each of them supports best. SuperSet presented itself as a great tool for data visualization with dashboards
...
Thanks to these two complementary UIs we were able to fulfill broad diversity of data access needs within our company.
...
the software when needed, analyze how the code was written in order to improve it or squash the bugs
which are inevitable in every piece of software. Unfortunately, Pivot is no longer an open source tool
since November 2016 when it has been announced on the
[mailing list](https://groups.google.com/forum/#!topic/imply-user-group/LaKKgXqWePQ) that
a [GitHub repository](https://github.com/implydata/pivot) of Pivot has been closed. As Pivot has become
a commercial software we have naturally decided to pay for the license granting us rights to use it. We had quite good
...
we have finally decided that we should have a visualization software tailored exactly to our needs.

## Rise of the Turnilo

This is how [Turnilo](https://github.com/allegro/turnilo) was born. The name we have chosen should ring the bell
...
which happened to be a Yahoo’s fork of Pivot. We have spent a lot of time analyzing how this software works
and how difficult it would be in order to add new features before we decided that we can adopt it.
We were glad that Pivot has been written in [TypeScript](https://www.typescriptlang.org/) as we like
strictly typed languages having background in Scala and earlier Java. We already had an idea what were
the biggest obstacles in using Pivot but we have also asked our users of the software what would they need
and what is not so important for them.
After getting feedback we have created a short [manifesto](https://github.com/allegro/turnilo#manifesto)
...
it is and adjust the codebase to your needs. When we have started preparing those simple changes it turned out
that it was not that easy to build Pivot and work with it efficiently. It was being built with a custom
...
We strongly believe that keeping the quality of the build tools is essential in getting on track
to efficient development. It is an investment which is well worth taking. Thus before taking a deep dive
into the codebase we wanted to improve the way we could develop.

We have started by removing custom Gulp scripts and switching to [npm](https://npmjs.com) scripts
...
*   Upgrading the [Imply Plywood](https://github.com/implydata/plywood) dependency has been the most time consuming
    but also the most important task because the library provides API for querying Druid cluster. As a fun fact
    Turnilo depended on a private Swiv’s version of Imply’s Plywood which was only
    ...
    and sustain the software so we have decided to devote some time to synchronize the code base
...
*   Preparing an [automatic release management](https://github.com/webpro/release-it) with
    generation of readable changelog which we were really missing in some other OSS projects.
*   Adding TypeScript lint rules in order to make sure that the code is both strictly typed and consistently styled.
    This was especially important as we welcome external contributors.

## Squashing the most evident bugs

Soon after Turnilo has been installed in production environment at Allegro we have started receiving feedback
from our users. It was mostly positive but there were a few complaints that repeated itself in most of the reports.
...
*   Expiring URLs.
    ...
    This meant that changes in one of Turnilo dependencies could invalidate all links. We have developed
    an optimized URL encoding with encapsulated Plywood expressions and full support for legacy URLs
    which allowed as making internal data model changes without sacrificing thousands of already bookmarked
    or shared links.
*   Flat representation of data cube facts.
    ...
    We have developed the feature for organizing dimensions and measures into hierarchy of nested groups.
*   No possibility to tune [approximate histogram](http://druid.io/docs/latest/development/extensions-core/approximate-histograms.html)
    queries. It appears that at querying time Apache Druid ignores properties of a histogram metrics
    ...
    Upgrading Plywood to the latest version has fixed the issue.
*   Confusion of what data is being visualised due to different time zones.
    We have started presenting selected time zone in the main screen.
*   Non intuitive export to Microsoft Excel.
    ...
    using CSV (comma separated values) export available in Turnilo. They were facing the problem that even though
    the exported files were opened in Excel they were not split into columns and presented as one column instead.
    The root of the problem stems probably from localisation issues that Microsoft engineers had when
    developing Excel for Polish locale. In Poland and a few other countries we use a comma as a decimal separator
    and that would most probably mess with the import. We were deliberating about convenient but still
    bulletproof solution for our users and we have decided to provide TSV (tab separated values)
...
We have fixed these issues and as a bonus have gained some experience in working with the software.
We were finally ready to plan and develop new features.

## What’s new?

With most urgent fixes out of the way we could start developing new features. We are mostly proud with a few of them
...
so that users could easily navigate from web component definition into well-crafted component usage report.
...
based on view definition payload encoded in JSON but it was not well documented.
...
Another feature that has been frequently requested by our users was the possibility to compare current metrics
with results observed day, week or year ago. That is how the Time Shift feature has been born. This feature
...
one of the predefined periods or entering custom period using
...
regarding the direction of changes in form of icons and colors. There is one interesting fact
that has been revealed with the Time Shift feature. Although for majority of measures value increase
means positive change, e.g. number of visitors and amount of revenue there are some measure where increase
...
The Time Shift feature is the most significant one developed for our users since
the incarnation of Turnilo but we are not going to rest on laurels yet...

## Plans for the future

It is worth a mention that Turnilo being available as an open source software helped us a lot. Only 10 days
after we have released the initial version on GitHub we already received some issues and nice discussion
on how to make it even better. Then it was encouraging to find out that Turnilo
[has already been deployed in Wikimedia Foundation](https://phabricator.wikimedia.org/T194427) even though
...
1.  [[157]](https://github.com/allegro/turnilo/issues/157) Making an interactive help with tutorial
...
Please keep in mind that Turnilo is an open source software and that you can be a part of it.
Feel free to create issue or even better pull request that align well with Turnilo’s
...
in [milestones](https://github.com/allegro/turnilo/milestones). We try hard to release new Turnilo version
