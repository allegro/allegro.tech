...
Instead, I want to focus on what I personally liked about both languages and how did it feel to transition between them.
...
just one statement by the debugger. The fact that query results a lazily evaluated also complicates stepping through 
the code a bit. 

After I switched to C#, Java 8 introduced a feature very similar to LINQ called 
[Streams](http://www.oracle.com/technetwork/articles/java/ma14-java-se-8-streams-2177646.html). Our previous example of 
filtering a list of users could look like this:

```java
List<User> result = users.stream().filter(user -> user.name.startsWith("A"));
```
...
features that help make the code more concise, like for example properties or optional parameters.

### Properties and Optional Parameters
Properties are an alternative to Java getters and setters. Having to add field accessors were one of the things that I 
liked the leased about Java. Sure, most Java IDEs can generate them on demand, but it’s still another action you need 
to do. The code also gets more cluttered. There are some libraries that let you add accessors by annotating a class, 
but this only really works if you IDE understands them. C# properties are built into the core language an offer a 
...
should be readonly. C# 6.0 went a step further allowing properties to be initialized inline.

Another feature that helps reduce code cluttering is allowing optional method arguments. This reduces the need for 
...
only verifies the use of generic types without removing them. Actual code generation is deferred until the class is 
loaded. At that point, the type parameter is replaced with the actual runtime type. Both approaches to generics have 
their advantages and disadvantages. I won’t go into too much detail here as there a lot of other resources that explain 
...
However, this solution doesn’t scale well with the number of classes and fields. If we expect the them to change often 
...
type is. To workaround this, in Java we need to define an additional Class parameter. We can then use it to explicitly 
...
a bit unintuitive the method already has a type parameter.

## From C# to Java
Over the 5 years I was using C# I really got used to convenience offered by these and other language features. At some 
point I decided it was time to change jobs and found an interesting position in Allegro. The primary language required 
...
### Spring
I never really used Spring before I switched to C#. I wanted to give it a try and was very impressed by it. Without any 
...
easy to get everything set up correctly without doing some research upfront. Spring definitely decreases the entry cost by 
hiding all of the configuration settings you don’t need at first. Later, if you need to, you can always configure them 
the way you want, but until then, the framework takes care of everything. 

Another thing I like about Sprig was the build in dependency container. It requires the user to just add a few annotations 
...
logic code with extra code that doesn’t really belong there. For example the below code creates a service endpoint that 
accept PUT requests, with a URL path parameter that’s not null and a request body that’s validated using annotations 
...
Spring then generates a class with query implementations based on the method names and injects it whenever you use 
the interface. Things get even more implicit if you want to add some custom queries. I read the documentations for 
this a few times and still didn’t quite believe the approach described there would just work. The solution was to add 
...
Spring wasn’t the only thing I liked about Java. Some core language features also caught my attention, like for example 
Optionals.
...
has a much more flexible mechanism for handling null values. For example, it let’s you define default values to use 
like this.
...
It also allows you transform the object or filter based on a predicate:
...
languages are also added to most of Microsoft’s services opening them developers that don’t use C#. Java is also picking 
up momentum by shortening the release cycle between versions and adding a lot of interesting features in it’s newest 
releases. With all that, the line between the two worlds becomes increasingly blurry and the choice between C# and Java 
will often boil down to personal preference.
...
confusing at first. Spring also gives Java a big advantage. And once one gets a bit more experience in programming, 
the language isn’t that important anymore. After all, it’s just a way to express our ideas. It’s a bit like with natural 
languages: you can write poetry no matter which one you use. What really important is the developer that turns an idea 
